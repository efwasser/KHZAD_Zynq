...
/********************************************************************************************************
  7. demonstration: This function is designed to demonstrate the hardware implementation correctness in a simple way.
  ...
  Since the function works block-by-block and switchs between encryption and decryption, 
  only ECB mode can be used for this demonstration.
*********************************************************************************************************/
void demonstration()
{
  u32 key1, key2, key3, key4;
  bool valid_answer, go = 1;
  u8 answer, data_string[MAX_LENGTH+1], decrypted_string[MAX_LENGTH+1], plain[BLOCKSIZEB], cipher[BLOCKSIZEB], decrypted[BLOCKSIZEB];
  printf("*************************************************************** \n\r");
	  ...
	  while (data_string[i])
	  {
		  ...
		  if (((j != BLOCKSIZEB) && !(data_string[i+1])) || (j == BLOCKSIZEB)) // need to encrypt
		  {
			  if (block_num == 0)
				...
				Zynq_crypt(plain, key1, key2, key3, key4, 0, 0, 1, 1, 0, 0, 0, cipher);   // not first operation: only_data = 1. enc_dec = 1.
...
/********************************************************************************************************
  ...
  The user can choose between encryption or decryption, 
...
void application()
{
  ...
  bool enc_dec, only_data, op_mode, first_block, new_IV, valid_answer, key_entered, IV_entered, first_run = 1, go = 1;
...
void about()
{
	...
	printf("In our design, the programmable logic (PL) is used for implementing the algorithm, in ECB mode or CBC mode, \n");
