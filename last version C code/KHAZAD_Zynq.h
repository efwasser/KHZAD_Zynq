...
8.  HW_application
9.  NESSIEencrypt_CBC
10. NESSIEdecrypt_CBC
11. SW_application
12. CBC_MAC
13. PRNG
14. random_vectros_test
15. PRNG_application
16. print_data
17. compare_blocks
18. about
...
/********************************************************************************************************
  ...
  combined with encryption and decryption on Zynq, to test the hardware implementation ECB results.
...
/********************************************************************************************************
  7. demonstration: this function is designed to demonstrate the hardware implementation correctness in a simple friendly way.
  ...
  while (go)
  {
	  ...
	  while (data_string[i])
	  {
		  ...
		  if (((j != BLOCKSIZEB) && !(data_string[i+1])) || (j == BLOCKSIZEB)) // need to encrypt
		  {
			  ...
			  if(compare_blocks(cipher, Zynq_cipher, BLOCKSIZE) != 0)
			    printf("** HW encryption error: **\n    Ciphertext from Zynq is different than the reference code cipher!\n");

			  if(compare_blocks(plain, Zynq_decrypted, BLOCKSIZE) != 0)
			    printf("** HW decryption error: **\n    Decrypted ciphertext on Zynq is different than the plaintext!\n");
	  ...
	  printf("The decrypted string from Zynq HW implementation is: %s \n\r", decrypted_string);
...
/********************************************************************************************************
  8. HW_application: a simple practical encryption/decryption application. The calculations are done on hardware (the Zynq PL side).
...
void HW_application()
{
  ...
  while (go)
  {
	...
	if ((!CBC_first_run) && (op_mode == 1))
	{
		valid_answer = 0;
		do {
			printf("Do you want to enter a new IV? Please answer y/n \n\r");
			scanf(" %c", &answer);
			if ((answer == 'y') || (answer == 'Y') || (answer == '1'))
				valid_answer = 1;
			else if ((answer == 'n') || (answer == 'N') || (answer == '0'))
	...
	if ((op_mode == 1) && ((CBC_first_run) || ((!CBC_first_run) && ((answer == 'y') || (answer == 'Y') || (answer == '1')))))
	{
		...
		printf("For your convenience, the IV is entered in 2 parts, each one up to 8 figures in hexadecimal base \n(leading zeros will be added). \n\r");
...
/********************************************************************************************************
  9. NESSIEencrypt_CBC: this function wraps the reference code "NESSIEencrypt" function with the 
...
/********************************************************************************************************
  10. NESSIEdecrypt_CBC: this function wraps the reference code "NESSIEdecrypt" function with the 
...
/********************************************************************************************************
  11. SW_application: a simple practical encryption/decryption application. The calculations are done on software (the Zynq PS side).
...
/********************************************************************************************************
  12. CBC_MAC (Message Authentication Code): 
...
void CBC_MAC()
{
  ...
  printf("*************************************************************** \n\r");
  printf("CBC-MAC generator \n");
  printf("using the KHAZAD algorithm \n\r");
  while (go)
  {
	key_entered = 0;
...
/********************************************************************************************************
  14. random_vectros_test: compares HW implementation results to SW implementation results for random vectors.
...
void random_vectros_test()
{
	...
	printf("*************************************************************** \n\r");
	printf("Please enter a number of random keys to use \n\r");
	scanf("%u", &keys_num);
	printf("Please enter a number of random messages to test for each key. Each message consists of %u data blocks. \n\r", BLOCKS_NUM);
...
/********************************************************************************************************
  15. PRNG_application: this function prints on screen a 64-bit cryptographically secure pseudo-random number.
...
void PRNG_application()
{
	...
	printf("*************************************************************** \n\r");
	printf("64-bit CSPRNG: Cryptographically Secure Pseudo-Random Number Generator \n");
	printf("using the KHAZAD algorithm in CTR mode \n\r");
	printf("CSPRNG seed: \n");
...
/********************************************************************************************************
  16. print_data: prints a given string "str", then "=", then the ASCII code in hexadecimal figures of 
...
/********************************************************************************************************
  17. compare_blocks: compares two u8 strings with the same given length.
  From the reference code bctestvectors.c file.
*********************************************************************************************************/
int compare_blocks(u8 *m1, u8 *m2, int len_bits)
{
...
/********************************************************************************************************
  18. about: prints information about this project.
...
void about()
{
	...
	printf("''The KHAZAD Legacy-Level Block Cipher'' is an involutional block cipher designed by Paulo S.L.M. Barreto and Vincent Rijmen.\n");
	printf("It has a substitution-permutation network (SPN) structure, \n");
	printf("and it uses a 128-bit key, operates on 64-bit data blocks, and comprises 8 rounds.\n");
	printf("An interesting feature of the algorithm is that the same round function is used for both the key schedule part and for encryption/decryption.\n");
	...
	printf("This implementation of KHAZAD uses the MicroZed 7010 development board by Avnet Inc., \n");
	...
	printf("In this project, implementation was done in two ways: software and hardware.\n");
	printf("In the software implementation, all the calculations are done using the Zynq processing system (PS).\n");
	printf("In the hardware implementation, the PS is used mainly for dealing with user input & output operations, \n");
	printf("and the programmable logic (PL) is used for implementing the algorithm and making the calculations.\n");
	printf("Both implementations include the basic ECB (Electronic Codebook) mode and the more complex CBC (Cipher Block Chaining) mode.\n");
	printf("The CBC mode is also used to generate basic CBC-MAC (Message Authentication Code).\n");
	printf("In addition, a CSPRNG (Cryptographically Secure Pseudo-Random Number Generator) was created, using the CTR mode principle.\n\r");
